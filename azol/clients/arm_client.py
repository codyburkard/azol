...
class ArmClient( OAuthHTTPClient ):
    ...
    def get( self, path, api_version=None ):
        ...
        """
        if api_version is None:
            logging.info("Attempting to resolve api version...")
            ...
            if "/providers" not in path:
                ...
                if len(namespace_and_provider_list) == 2:
                    provider = namespace_and_provider_list[1].lower()
                else:
                    provider = "/".join(namespace_and_provider_list[1:-2]).lower()
                print(provider)
